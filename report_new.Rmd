...
author: Yu Luo, Khushee Thakker, Yunqi Chen
output:
...
# The Six Variables Might Affected Happiness
...
The above table \@ref(tab:tabfree) is the average "freedom to make life choices" score among all the regions. It is divided into three groups by the average score greater than 0.80, 0.75-0.80, and lower than 0.75.
...
As the figure \@ref(fig:freeh) above, in the group of scores higher than 8.0, except Southeast Asia, the ladder scores in the other three regions are higher than the average.
...
The above figure \@ref(fig:freem) is the group of scores of 0.75-0.80. The distribution of the points is generally surrounding the average line. However, in South Asia, most of the points are lower than the average.
...
From the above figure \@ref(fig:freel), in the group of scores less than 0.75, a tremendous amount of the points of Sub-Saharan Africa are on the left of the average line. However, the points are scattered evenly from high to low in the Middle East and North Africa. 
...
The table below \@ref(tab:tabgene) is the average generosity score among all the regions. It is divided into two groups by the average score over and below 0.
...
The table below \@ref(tab:tabcor) is the average "perceptions of corruption" score among all the regions. It is divided into three groups by the average score greater than 0.75, 0.70-0.75, lower than 0.70. 
...
From the figure \@ref(fig:corl), in the group of scores less than 0.70, not surprisingly, all the points are on the right of the average ladder score line. 
...
From the figure \@ref(fig:corm), in the group of scores with between 0.70 and 0.80, except South Asia and Sub-Saharan Africa shows lower ladder score, the other regions all indicate a middle ladder score.
...
The above figure \@ref(fig:corh) is the group of scores greater than 0.80. In Central and Eastern Europe, and Latin America and Caribbean, most of the points are on the right of the average line.
...
+ The below table \@ref(tab:countab) shows the Countries according to their happiness rank. We have divided them into levels, Top5, Middle5 and Worst5 Countries.
...
```{r countab}
comparison.score %>% 
  filter(columns == "happiness_score") %>%
  select(country_name,regional_indicator,Level,happiness_rank,score) %>% 
  kable(caption = "Top 5, Middle5 and Worst 5 Countries") %>%
   kable_styling(c("hover", 
                  "condensed",
                  position = "center", 
                  fixed_thead = TRUE))
...
+ From the above map \@ref(fig:map) you can easily spot the happiest country, Finland. <br>
